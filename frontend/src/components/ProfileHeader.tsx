...
const ProfileHeader: React.FC<ProfileHeaderProps> = ({ userData, onSave, isOwnProfile }) => {
	const [isEditing, setIsEditing] = useState(false);
	const [editedData, setEditedData] = useState<Partial<IUser>>({});

	const { data: authUser } = useAuthUser();
	const { data: connectionStatus } = useConnectionStatus(isOwnProfile ? undefined : userData._id);

	const isConnected = Array.isArray(userData.connections)
		? userData.connections.some((connection: any) =>
				typeof connection === "string" ? connection === authUser?._id : connection?._id === authUser?._id
		  )
		: false;

	const { mutate: sendConnectionRequest } = useSendConnectionRequest(userData._id);
	...
	const handleImageChange = (event: React.ChangeEvent<HTMLInputElement>) => {
		...
		if (file) {
			...
			reader.onloadend = () => {
				setEditedData((prev) => ({ ...prev, [event.target.name]: reader.result as string }));
			};
				...
				<div className='text-center mb-4'>
					{isEditing ? (
						<input
							...
							className='text-2xl font-bold mb-2 text-center w-full border rounded p-1'
						/>
					) : (
					...
					{isEditing ? (
						<input
							...
							className='text-gray-600 text-center w-full border rounded p-1'
						/>
					) : (
